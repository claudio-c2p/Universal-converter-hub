...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/sqlite-to-csv',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.csv'),
  });
  return (
    <ToolLayout title="SQLITE → CSV" description="Converta seu arquivo SQLITE para CSV de forma rápida e gratuita." category="Banco de Dados"
