...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/dbf-to-json',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.json'),
  });
  return (
    <ToolLayout title="DBF → JSON" description="Converta seu arquivo DBF para JSON de forma rápida e gratuita." category="Banco de Dados"
