...
export default function VideoConvertPage() {
  ...
  const [to, setTo] = useState<(typeof FORMATS)[number]>('mp4');
  ...
  async function handleConvert() {
    ...
    try {
      ...
      form.append('to', to);
      const res = await fetch('/api/audio-video/video-convert', { method: 'POST', body: form });
      if (!res.ok) {
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (status.status === 'done') {
          ...
          pushHistoryEntry({ toolTitle: 'Conversor de Vídeo', fileName, downloadUrl });
          setPhase('done');
        } else if (status.status === 'error') {
          stopPolling();
          setError(status.error ?? 'Falha ao converter o vídeo.');
          setPhase('error');
        }
      }, 2500);
    } catch {
  ...
  return (
    <ToolLayout title="Conversor de Vídeo" description="Converta entre MP4, WEBM e MOV" category="Áudio e Vídeo">
      <FileDropzone accept=".mp4,.webm,.mov,.mkv,.avi" maxSizeMB={200} onFileSelect={setFile} />
      ...
      >
        {phase === 'uploading' ? 'Enviando…' : phase === 'processing' ? 'Processando (vídeos grandes podem levar minutos)…' : 'Converter'}
      </button>
