...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/extra/djvu-to-txt',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.txt'),
  });
  return (
    <ToolLayout title="DJVU → TXT" description="Converta seu arquivo DJVU para TXT de forma rápida e gratuita." category="eBook"
      ...
      <FileDropzone accept=".djvu" maxSizeMB={40} onFileSelect={setFile} label="Arraste o arquivo aqui" />
