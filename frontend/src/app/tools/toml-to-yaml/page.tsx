...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/toml-to-yaml',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.yaml'),
  });
  return (
    <ToolLayout title="TOML → YAML" description="Converta seu arquivo de forma rápida e gratuita." category="Dados" icon={<ToolIcon d="M10 20l4-16M6 16l-4-4 4-4M18 8l4 4-4 4" />}>
      <FileDropzone accept=".toml" maxSizeMB={20} onFileSelect={setFile} label="Arraste o arquivo aqui" />
