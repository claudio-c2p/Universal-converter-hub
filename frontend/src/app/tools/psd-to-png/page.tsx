...
export default function PsdToPngPage() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const res = await fetch('/api/image-extra/psd-to-png', { method: 'POST', body: form });
      if (!res.ok) {
      ...
      pushHistoryEntry({ toolTitle: 'PSD → PNG', fileName, downloadUrl });
    } catch {
  ...
  return (
    <ToolLayout title="PSD → PNG" description="Converta a imagem composta de um arquivo PSD (Photoshop) para PNG" category="Imagem">
      <FileDropzone accept=".psd" maxSizeMB={40} onFileSelect={setFile} />
      <button
