...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/extra/mobi-to-epub',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.epub'),
  });
  return (
    <ToolLayout title="MOBI → EPUB" description="Converta seu arquivo MOBI para EPUB de forma rápida e gratuita." category="eBook"
      ...
      <FileDropzone accept=".mobi" maxSizeMB={40} onFileSelect={setFile} label="Arraste o arquivo aqui" />
