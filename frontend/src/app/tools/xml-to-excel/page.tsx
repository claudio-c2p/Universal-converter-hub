...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/xml-to-excel',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.xlsx'),
  });
  return (
    <ToolLayout title="XML → EXCEL" description="Converta seu arquivo XML para XLSX de forma rápida e gratuita." category="Dados"
