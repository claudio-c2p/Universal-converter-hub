...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/json-to-yaml',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.yaml'),
  });
  return (
    <ToolLayout title="JSON → YAML" description="Converta seu arquivo de forma rápida e gratuita." category="Dados" icon={<ToolIcon d="M10 20l4-16M6 16l-4-4 4-4M18 8l4 4-4 4" />}>
