...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/extra/cbz-to-epub',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.epub'),
  });
  return (
    <ToolLayout title="CBZ → EPUB" description="Converta seu arquivo CBZ para EPUB de forma rápida e gratuita." category="eBook"
      ...
      <FileDropzone accept=".cbz" maxSizeMB={40} onFileSelect={setFile} label="Arraste o arquivo aqui" />
