...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/extra/chm-to-epub',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.epub'),
  });
  return (
    <ToolLayout title="CHM → EPUB" description="Converta seu arquivo CHM para EPUB de forma rápida e gratuita." category="eBook"
      ...
      <FileDropzone accept=".chm" maxSizeMB={40} onFileSelect={setFile} label="Arraste o arquivo aqui" />
