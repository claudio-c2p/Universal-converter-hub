...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/libreoffice/excel-to-ods',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.ods'),
  });
  return (
    <ToolLayout title="EXCEL → ODS" description="Converta seu arquivo XLSX para ODS de forma rápida e gratuita." category="Office"
      ...
      <FileDropzone accept=".xlsx,.xls" maxSizeMB={25} onFileSelect={setFile} label="Arraste o arquivo aqui" />
