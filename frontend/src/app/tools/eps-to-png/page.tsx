...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/binary-tools/eps-to-png',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.png'),
  });
  return (
    <ToolLayout title="EPS → PNG" description="Converta seu arquivo EPS para PNG de forma rápida e gratuita." category="Documento"
      icon={<ToolIcon d="M7 8h10M7 12h6m-6 4h4M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z" />}>
      <FileDropzone accept=".eps,.ps" maxSizeMB={20} onFileSelect={setFile} label="Arraste o arquivo aqui" />
