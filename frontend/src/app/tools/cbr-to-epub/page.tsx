...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/extra/cbr-to-epub',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.epub'),
  });
  return (
    <ToolLayout title="CBR → EPUB" description="Converta seu arquivo CBR para EPUB de forma rápida e gratuita." category="eBook"
      ...
      <FileDropzone accept=".cbr" maxSizeMB={40} onFileSelect={setFile} label="Arraste o arquivo aqui" />
