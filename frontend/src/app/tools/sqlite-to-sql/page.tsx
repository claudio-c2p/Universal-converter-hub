...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/sqlite-to-sql',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.sql'),
  });
  return (
    <ToolLayout title="SQLITE → SQL" description="Converta seu arquivo SQLITE para SQL de forma rápida e gratuita." category="Banco de Dados"
