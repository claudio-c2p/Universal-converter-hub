...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/json-to-pdf',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.pdf'),
  });
  return (
    <ToolLayout title="JSON → PDF" description="Converta seu arquivo JSON para PDF de forma rápida e gratuita." category="Dados"
      ...
      <FileDropzone accept=".json" maxSizeMB={5} onFileSelect={setFile} label="Arraste o arquivo aqui" />
