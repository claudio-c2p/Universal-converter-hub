...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/xml-to-yaml',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.yaml'),
  });
  return (
    <ToolLayout title="XML → YAML" description="Converta seu arquivo de forma rápida e gratuita." category="Dados" icon={<ToolIcon d="M10 20l4-16M6 16l-4-4 4-4M18 8l4 4-4 4" />}>
      <FileDropzone accept=".xml" maxSizeMB={20} onFileSelect={setFile} label="Arraste o arquivo aqui" />
