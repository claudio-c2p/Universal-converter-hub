...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/data-convert/csv-to-pdf',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.pdf'),
  });
  return (
    <ToolLayout title="CSV → PDF" description="Converta seu arquivo CSV para PDF de forma rápida e gratuita." category="Dados"
      ...
      <FileDropzone accept=".csv" maxSizeMB={5} onFileSelect={setFile} label="Arraste o arquivo aqui" />
