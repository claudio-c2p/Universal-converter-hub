...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/vcf-to-json',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.json'),
  });
  return (
    <ToolLayout title="VCF → JSON" description="Converta seu arquivo VCF para JSON de forma rápida e gratuita." category="Utilitário"
