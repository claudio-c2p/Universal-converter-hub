...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/dbf-to-csv',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.csv'),
  });
  return (
    <ToolLayout title="DBF → CSV" description="Converta seu arquivo DBF para CSV de forma rápida e gratuita." category="Banco de Dados"
