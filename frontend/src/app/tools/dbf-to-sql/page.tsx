...
export default function Page() {
  ...
  const { status, errorMsg, convert } = useConverter({
    endpoint: '/api/file-db/dbf-to-sql',
    outputFilename: (name) => name.replace(/\.[^.]+$/, '.sql'),
  });
  return (
    <ToolLayout title="DBF → SQL" description="Converta seu arquivo DBF para SQL de forma rápida e gratuita." category="Banco de Dados"
      ...
      <FileDropzone accept=".dbf" maxSizeMB={10} onFileSelect={setFile} label="Arraste o arquivo aqui" />
